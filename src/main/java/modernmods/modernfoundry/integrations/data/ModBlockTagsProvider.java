...
public class ModBlockTagsProvider extends IntrinsicHolderTagsProvider<Block> {
    ...
    public ModBlockTagsProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider, ExistingFileHelper existingFileHelper) {
        super(output, Registries.BLOCK, lookupProvider, (block) -> block.builtInRegistryHolder().key(), TConstruct.MOD_ID, existingFileHelper);
    }

    @Override
    public @NotNull String getName() {
        return "TciIntegration - Block Tags";
    }

    @Override 
    protected void addTags(HolderLookup.@NotNull Provider provider) {
        this.tag(TagManager.Blocks.BRONZE).add(TciItems.BRONZE.get());
