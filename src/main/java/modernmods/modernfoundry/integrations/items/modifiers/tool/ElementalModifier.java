...
import modernmods.modernfoundry.library.recipe.modifiers.severing.SeveringRecipeCache;
...
public class ElementalModifier extends ManaModifier implements MeleeHitModifierHook, ProcessLootModifierHook {
    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        ...
        if (player != null && !player.level().isClientSide) {
            ...
            if (TConstruct.RANDOM.nextFloat() <= 0.05F) {
            ...
            if (generatedLoot.stream().noneMatch(stack -> stack.is(ItemTags.SKULLS))) {
                ...
                if (!recipes.isEmpty()) {
                    ...
                    for (SeveringRecipe recipe : recipes) {
                        ...
                        if (!result.isEmpty() && TConstruct.RANDOM.nextFloat() <= chance) {
                            if (result.getCount() > 1) {
                                result.setCount(TConstruct.RANDOM.nextInt(result.getCount()) + 1);
                            }
