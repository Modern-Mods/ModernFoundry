...
public class TerrestrialModifier extends Modifier implements InventoryTickModifierHook, EquipmentChangeModifierHook {
    ...
    public void onEquip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        ...
        if (player != null && !player.level().isClientSide) {
            ...
            CapabilityRegistry.getBotania(sp).ifPresent(data -> {
                ...
                TinkerNetwork.getInstance().sendTo(new BotaniaSetData(hasSet, data.hasGreatFairy(), data.hasAlfheim()), sp);
            });
        }
    }
    ...
    public void onUnequip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        ...
        if (player != null && !player.level().isClientSide) {
            ...
            CapabilityRegistry.getBotania(sp).ifPresent(data -> {
                ...
                TinkerNetwork.getInstance().sendTo(new BotaniaSetData(false, data.hasGreatFairy(), data.hasAlfheim()), sp);
            });
        }
    }
    ...
    public void onInventoryTick(IToolStackView tool, ModifierEntry modifier, Level world, LivingEntity holder, int itemSlot, boolean isSelected, boolean isCorrectSlot, ItemStack stack) {
        ...
        if (player != null && !player.level().isClientSide) {
            ...
            if (sp.tickCount % 20 == 0
                    && tool.getDamage() > 0
                    && BotaniaHelper.requestManaExactForTool(stack, sp, getManaPerDamage(sp) * 2, true)) {
    ...
    public boolean hasArmorSetItem(ServerPlayer sp, EquipmentSlot slot) {
        ...
        return armor.getUpgrades().getLevel(TciModifiers.TERRESTRIAL_MODIFIER.getId()) > 0;
    }
