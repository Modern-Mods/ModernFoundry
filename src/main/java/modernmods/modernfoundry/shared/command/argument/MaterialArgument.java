...
public class MaterialArgument implements ArgumentType<IMaterial> {
  ...
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    return TinkerSuggestionProvider.suggestResource(TConstruct.MOD_ID, MaterialRegistry.getInstance().getAllMaterials().stream().map(IMaterial::getIdentifier), builder, MaterialId::getIdentifier, MaterialTooltipCache::getColoredDisplayName);
  }
