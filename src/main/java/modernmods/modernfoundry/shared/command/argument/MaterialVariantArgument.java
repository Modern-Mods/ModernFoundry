...
public class MaterialVariantArgument implements ArgumentType<MaterialVariantId> {
  ...
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    return TinkerSuggestionProvider.suggestResource(TConstruct.MOD_ID, MaterialRegistry.getInstance().getAllMaterials().stream().map(IMaterial::getIdentifier), builder, id -> id.getIdentifier(), MaterialTooltipCache::getColoredDisplayName);
  }
