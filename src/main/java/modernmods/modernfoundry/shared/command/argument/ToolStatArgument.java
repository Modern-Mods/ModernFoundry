...
public class ToolStatArgument<T extends IToolStat> implements ArgumentType<T> {
  private static final Collection<String> EXAMPLES = Arrays.asList("modernfoundry:mining_speed", "modernfoundry:durability");
  ...
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    return TinkerSuggestionProvider.suggestResource(TConstruct.MOD_ID, ToolStats.getAllStats().stream().filter(filter::isInstance), builder, stat -> stat.getName().getIdentifier(), IToolStat::getPrefix);
  }
