...
import modernmods.modernfoundry.plugin.jei.casting.CastingBasinCategory;
import modernmods.modernfoundry.plugin.jei.casting.CastingTableCategory;
import modernmods.modernfoundry.plugin.jei.entity.DefaultEntityMeltingRecipe;
import modernmods.modernfoundry.plugin.jei.entity.EntityMeltingRecipeCategory;
import modernmods.modernfoundry.plugin.jei.entity.SeveringCategory;
...
import modernmods.modernfoundry.plugin.jei.melting.FoundryCategory;
import modernmods.modernfoundry.plugin.jei.melting.MeltingCategory;
import modernmods.modernfoundry.plugin.jei.melting.MeltingFuelHandler;
import modernmods.modernfoundry.plugin.jei.modifiers.ModifierBookmarkIngredientRenderer;
import modernmods.modernfoundry.plugin.jei.modifiers.ModifierIngredientHelper;
import modernmods.modernfoundry.plugin.jei.modifiers.ModifierRecipeCategory;
import modernmods.modernfoundry.plugin.jei.modifiers.ModifierWorktableCategory;
import modernmods.modernfoundry.plugin.jei.modifiers.SlotIngredientHelper;
import modernmods.modernfoundry.plugin.jei.modifiers.SlotIngredientRenderer;
import modernmods.modernfoundry.plugin.jei.partbuilder.MaterialItemList;
import modernmods.modernfoundry.plugin.jei.partbuilder.PartBuilderCategory;
import modernmods.modernfoundry.plugin.jei.partbuilder.PatternIngredientHelper;
import modernmods.modernfoundry.plugin.jei.partbuilder.PatternIngredientRenderer;
import modernmods.modernfoundry.plugin.jei.transfer.CraftingStationTransferInfo;
import modernmods.modernfoundry.plugin.jei.transfer.TinkerStationTransferInfo;
import modernmods.modernfoundry.plugin.jei.transfer.ToolInventoryTransferInfo;
import modernmods.modernfoundry.plugin.jei.util.GuiContainerTankHandler;
import modernmods.modernfoundry.plugin.jei.util.PotionSubtypeInterpreter;
import modernmods.modernfoundry.plugin.jei.util.ToolPartSubtypeInterpreter;
import modernmods.modernfoundry.plugin.jei.util.ToolSubtypeInterpreter;
...
public class JEIPlugin implements IModPlugin {
  ...
  public void registerIngredients(IModIngredientRegistration registration) {
    ...
    registration.register(TConstructJEIConstants.MODIFIER_TYPE, modifiers, new ModifierIngredientHelper(), ModifierBookmarkIngredientRenderer.INSTANCE, new LoadableCodec<>(ModifierEntry.LOADABLE));
    registration.register(TConstructJEIConstants.PATTERN_TYPE, Collections.emptyList(), new PatternIngredientHelper(), PatternIngredientRenderer.INSTANCE, Identifier.CODEC.xmap(Pattern::new, Pattern::getIdentifier));
    ...
    registration.register(TConstructJEIConstants.SLOT_TYPE, slots, new SlotIngredientHelper(), SlotIngredientRenderer.INGREDIENT, new LoadableCodec<>(SlotType.SlotCount.LOADABLE));
  }
  ...
  private static void addCastingCatalyst(IRecipeCatalystRegistration registry, ItemLike item, mezz.jei.api.recipe.RecipeType<IDisplayableCastingRecipe> ownCategory, RecipeType<MoldingRecipe> type) {
    ...
    if (!RecipeHelper.getRecipes(ClientRecipeCache.getRecipeMap(), type).isEmpty()) {
  ...
  public void onRuntimeAvailable(IJeiRuntime jeiRuntime) {
    ...
    // hide easter egg recipes; source vanilla crafting recipes from JEI's own synced data (there is no client RecipeManager in 26.1)
    Set<Identifier> easterEggIds = Set.of(EASTER_EGG_RECIPES);
    IRecipeManager recipeManager = jeiRuntime.getRecipeManager();
    List<RecipeHolder<CraftingRecipe>> easterEggs = recipeManager.createRecipeLookup(RecipeTypes.CRAFTING)
      .includeHidden().get()
      .filter(holder -> easterEggIds.contains(holder.id().identifier()))
      .toList();
    if (!easterEggs.isEmpty()) {
      recipeManager.hideRecipes(RecipeTypes.CRAFTING, easterEggs);
    }
