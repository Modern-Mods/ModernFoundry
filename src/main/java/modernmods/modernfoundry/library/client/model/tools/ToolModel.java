...
import modernmods.hilt.data.loadable.mapping.CompactLoadable;
...
import modernmods.hilt.util.ReversedListBuilder;
...
public class ToolModel implements IUnbakedGeometry<ToolModel> {
  ...
  private static IModelBuilder<?> makeModelBuilder(IGeometryBakingContext context, ItemOverrides overrides, TextureAtlasSprite particle) {
    return IModelBuilder.of(context.useAmbientOcclusion(), false, false, context.getTransforms(), overrides, particle, HiltItemLayerModel.getDefaultRenderType(context));
  }
  ...
  private static BakedModel bakeInternal(IGeometryBakingContext owner, Function<Material, TextureAtlasSprite> spriteGetter, @Nullable Transformation largeTransforms,
    ...
    for (int i = parts.size() - 1; i >= 0; i--) {
      ...
      if (part.hasMaterials()) {
        ...
        smallQuads.add(HiltItemLayerModel.getQuadsForSprite(materialSprite.color(), -1, materialSprite.sprite(), smallTransforms, materialSprite.emissivity(), smallPixels));
        if (largeTransforms != null) {
        ...
        // same drill as above
        smallQuads.add(HiltItemLayerModel.getQuadsForSprite(-1, -1, particle, smallTransforms, 0, smallPixels));
        if (largeTransforms != null) {
          largeQuads.add(HiltItemLayerModel.getQuadsForSprite(-1, -1, spriteGetter.apply(owner.getMaterial(part.getName(true))), largeTransforms, 0, largePixels));
        }
      }
    }
